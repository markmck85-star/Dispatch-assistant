...
exports.handler = async () => {
  ...
  const [{ data: sites, error: sitesErr }, { data: rows, error: rowsErr }] = await Promise.all([
    ...
    supabase.from('site_visits')
      .select('state, account_name_raw, appointment_number, started_at')
  ...
  for (const r of rows) {
    ...
    if (!groups[key]) groups[key] = { state: r.state, account_name_raw: r.account_name_raw, row_count: 0, saNumbers: [] };
    groups[key].row_count++;
    // Rows already come back most-recent-first (started_at desc), so the
    // first few appointment_numbers seen per group are the most recent --
    // capped at 5 so a Salesforce search has options without cluttering
    // the card. Skips blanks and de-dupes (a site can have several
    // needs_review rows sharing one SA number in edge cases).
    if (r.appointment_number && groups[key].saNumbers.length < 5 && !groups[key].saNumbers.includes(r.appointment_number)) {
      groups[key].saNumbers.push(r.appointment_number);
    }
  }
