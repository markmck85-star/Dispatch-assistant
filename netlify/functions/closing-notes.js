...
// 2026-09-15: second restock-detection signal, alongside the existing
// ticket-text inference in perform-import.js -- see detect-restock-in-note.js
// for the heuristic, confidence tiers, and why this feeds the same
// `included_restock` column.
const { classifyRestockConfirmation } = require('./detect-restock-in-note');
...
async function getSaNumbersNeedingNotes(supabase, daysBack, limit, priorityState) {
  ...
  const baseQuery = () => supabase
    .from('site_visits')
    .select('appointment_number, closing_note_attempts, is_restock, included_restock, included_restock_source')
  ...
  const toTargets = (rows) => (rows || []).map((r) => ({
    ...
    isRestock: !!r.is_restock,
    includedRestock: !!r.included_restock,
    includedRestockSource: r.included_restock_source || null,
  }));
...
//
// restockFlags (added 2026-09-15): { isRestock, includedRestock,
// includedRestockSource } from the same query that built the target list.
// When a captured note is classified by classifyRestockConfirmation() and
// the visit isn't already flagged as a restock (by remediation type OR the
// existing ticket-text-inference mechanism):
//   - HIGH confidence auto-sets included_restock=true, same as the
//     ticket-text mechanism -- get-restock-schedule.js's existing
//     `is_restock || included_restock` check picks it up with no other
//     change needed.
//   - LOW confidence does NOT touch included_restock -- instead sets
//     restock_review_pending=true + restock_review_reason, which surfaces
//     as a dispatcher-facing toast on restock-tracker.html
//     (get-restock-review-queue.js / resolve-restock-review.js) rather
//     than silently guessing on an ambiguous note.
// If included_restock was already true via the ticket-text mechanism, the
// source is combined rather than overwritten, so both signals stay visible.
async function recordAttempt(supabase, appointmentNumber, currentAttempts, note, dryRun, isBlank = false, restockFlags = {}) {
  const update = {
  ...
  if (note) {
    ...
    const { isRestock, includedRestock, includedRestockSource } = restockFlags;
    const alreadyRestock = isRestock || includedRestock;
    if (!alreadyRestock) {
      const classification = classifyRestockConfirmation(note);
      if (classification && classification.confidence === 'high') {
        update.included_restock = true;
        update.included_restock_source = 'closing_note';
      } else if (classification && classification.confidence === 'low') {
        update.restock_review_pending = true;
        update.restock_review_reason = classification.reason;
      }
    } else if (includedRestockSource === 'inferred') {
      const classification = classifyRestockConfirmation(note);
      if (classification && classification.confidence === 'high') {
        update.included_restock_source = 'inferred+closing_note';
      }
    }
  }
  if (isBlank) {
...
async function runClosingNotesPass(page, supabase, options = {}) {
  ...
  try {
    ...
    for (const target of targets) {
      const { appointmentNumber: saNumber, attempts, isRestock, includedRestock, includedRestockSource } = target;
      const restockFlags = { isRestock, includedRestock, includedRestockSource };
      ...
      try {
        ...
        if (!found) {
          ...
          if (!note || !note.trim()) {
            ...
            await recordAttempt(supabase, saNumber, attempts, note, dryRun, false, restockFlags);
